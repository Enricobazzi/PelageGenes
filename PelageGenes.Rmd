...
A table is created with the information regarding the SNPs. Different approaches have been tried. The final version [step 6b] has the advantage of having a fixed number of columns for the SNPEFF annotations (the maximum present in the file, with the ones missing filled with NONE), and a unique column for all of the CUSTOM annotations containing the information regarding the "feature" (region where the SNP falls). SNPs with no CUSTOM annotation will have "EMPTY" in that column.

## 6 - Create a table with the useful SNPs information and annotations
