...
# Divide by species creating a screen for long GATK process
...
## Try with awk fill - USELESS - see below (next ## step) for better version
...
# add "na|na|na|na|na|na|na|na|na|na|na|na|na|na|na|" to missing ANN columns to render the file squared
max=$(awk -v FS=',' 'max < NF { max = NF } END { print max }' lp_GATK_ParentIDsGrep_noIndividuals_nas_ordered.vcf)

awk -v max=$max -v FS=',' -v OFS=',' '{ for(i=NF+1; i<=max; i++) $i = "na|na|na|na|na|na|na|na|na|na|na|na|na|na|na|"; print }' lp_GATK_ParentIDsGrep_noIndividuals_nas_ordered.vcf > lp_GATK_ParentIDsGrep_noIndividuals_nas_ordered_quad.vcf

# change all field separators to TAB
tr ';' '\t' <lp_GATK_ParentIDsGrep_noIndividuals_nas_ordered_quad.vcf | tr ',' '\t' > lp_GATK_ParentIDsGrep_noIndividuals_nas_ordered_quad_tabs.vcf
...
while read pattern
do
sed 's/'$pattern'=//' lp_GATK_ParentIDsGrep_noIndividuals_nas_ordered_quad_tabs.vcf > tmp
mv tmp lp_GATK_ParentIDsGrep_noIndividuals_nas_ordered_quad_tabs.vcf
done < info.txt

# header will need to be added by hand (not all INFO is present in file and adding it all to the headers will mean having mismatching variable-value couples). The ones present in the file are CHROM	POS	ID	REF	ALT	QUAL	FILTER	AA	AC	AF	AN	BaseQRankSum	ClippingRankSum	DP	ExcessHet	FS	InbreedingCoeff	MQ	MQRankSum	QD	ReadPosRankSum	SOR	ANN1	ANN2	ANN3	ANN4	ANN5	ANN6
touch headers.txt
nano headers.txt

nano lp_GATK_ParentIDsGrep_noIndividuals_nas_ordered_quad_tabs.vcf # to add them: saved as lp_GATK_ParentIDsGrep_noIndividuals_nas_ordered_quad_tabs_headers.vcf


## FINAL VERSION - fixed number of SNPEFF and Custom annotations for each SNP

# divide the table in two - one with SNPEFF only and other with custom only -> render them quadratic and then unite them
...
# remove custom annotations - SNPEFF ONLY are left
...
# add "NONE" to missing ANN columns to render the file squared for NOCUSTOM FILE - SNPEFF ONLY
...
# isolate custom annotations - CUSTOM ONLY

# get all lines with custom annotations and the corresponding line number
...
# generate a file with the corresponding line number for the custom annotations
...
# add a white line in the custom annotations grep file in the missing positions
NoCustomLines=$(cat aa_nocustomlines)

# CAREFUL AS IT OVERWRITES - CHECK IF DONE ALREADY / OTHER POSSIBLE PROBLEMS - if problems are found better re-generate lp_GATK_ParentIDsGrep_noIndividuals_nas_ordered_customonly.vcf from scratch
...
# NOT NECESSARY - square before pasting - WILL CONDENSE ALL INFORMATION IN UNIQUE CUSTOM COLUMN (see next step)
max=$(awk -v FS=',' 'max < NF { max = NF } END { print max }' lp_GATK_ParentIDsGrep_noIndividuals_nas_ordered_customonly_blanklines.vcf)

awk -v max=$max -v FS=',' -v OFS=',' '{ for(i=NF+1; i<=max; i++) $i = "na|na|na|na|na|na|na|na|na|na|na|na|na|na|na|"; print }' lp_GATK_ParentIDsGrep_noIndividuals_nas_ordered_customonly_blanklines.vcf > lp_GATK_ParentIDsGrep_noIndividuals_nas_ordered_customonly_blanklines_squared.vcf

# condense all information in unique custom column: first count the maximum number of custom annotation columns (max); then cut columns 7, 22, 37, 52, 67 ecc. (always add 15 to the previous starting from 7) a number of times equal to max (to catch all)

max=$(awk -v FS=',' 'max < NF { max = NF } END { print max }' lp_GATK_ParentIDsGrep_noIndividuals_nas_ordered_customonly_blanklines.vcf) && echo $max # count columns

cut -d '|' -f7,22,37,52 lp_GATK_ParentIDsGrep_noIndividuals_nas_ordered_customonly_blanklines.vcf > lp_GATK_ParentIDsGrep_noIndividuals_nas_ordered_customonly_blanklines_condensed.vcf

# insert EMPTY in empty lines
...
# remove other separators -> tabs only
...
# header will need to be added by hand (not all INFO is present in file and adding it all to the headers will mean having mismatching variable-value couples). The ones present in the file are CHROM	POS	ID	REF	ALT	QUAL	FILTER	AA	AC	AF	AN	BaseQRankSum	ClippingRankSum	DP	ExcessHet	FS	InbreedingCoeff	MQ	MQRankSum	QD	ReadPosRankSum	SOR	SNPEFF1	SNPEFF2	CUSTOM (number of SNPEFF and CUSTOM columns can be seen by echoing $max while squaring the respective tables)
touch headers.txt
nano headers.txt
